...
describe('CorpseSections DB', () => {
  ...
  describe('find', () => {
    test('should return found section', () => (
      corpseSections.find(db, '123456789101').then((section) => {
  ...
  describe('addDrawer', () => {
    test('should update section with drawer attribute', () => {
      corpseSections.addDrawer(db, '123456789101', { name: 'Bozo', _id: '1' })
      .then((section) => {
        expect(section).not.toBeNull()
        expect(section.drawer).not.toBeNull()
        expect(section.drawer.name).toBe('Bozo')
      })
    })
    test('should update corpse record', () => {
      return corpseSections.addDrawer(db, '123456789101', { name: 'Bozo', _id: '1' })
      .then(() => (
